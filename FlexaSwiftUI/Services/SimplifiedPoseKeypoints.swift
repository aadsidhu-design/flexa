...
struct SimplifiedPoseKeypoints {
    ...
    let leftWristConfidence: Float
    let rightWristConfidence: Float
    ...
    // Explicit initializer to ensure call sites with labeled arguments match reliably.
    init(
        ...
        leftEye: CGPoint?,
        rightEye: CGPoint?,
        leftShoulder3D: simd_float3?,
        rightShoulder3D: simd_float3?,
        leftElbow3D: simd_float3?,
        rightElbow3D: simd_float3?,
        leftShoulderConfidence: Float,
        rightShoulderConfidence: Float,
        leftElbowConfidence: Float,
        rightElbowConfidence: Float,
        // MediaPipe call sites expect wrist confidences later in the argument list; accept them here
        leftWristConfidence: Float,
        rightWristConfidence: Float,
        noseConfidence: Float,
        neckConfidence: Float
    ) {
        ...
        self.leftShoulder3D = leftShoulder3D
        self.rightShoulder3D = rightShoulder3D
        self.leftElbow3D = leftElbow3D
        self.rightElbow3D = rightElbow3D
        self.leftShoulderConfidence = leftShoulderConfidence
        self.rightShoulderConfidence = rightShoulderConfidence
        self.leftElbowConfidence = leftElbowConfidence
        self.rightElbowConfidence = rightElbowConfidence
        self.leftWristConfidence = leftWristConfidence
        self.rightWristConfidence = rightWristConfidence
        self.noseConfidence = noseConfidence
        self.neckConfidence = neckConfidence
    }

    /// Convenience initializer with default confidences (0.0) for simpler call sites
    init(
        timestamp: TimeInterval,
        leftWrist: CGPoint?, rightWrist: CGPoint?, leftElbow: CGPoint?, rightElbow: CGPoint?, leftShoulder: CGPoint?, rightShoulder: CGPoint?, nose: CGPoint?, neck: CGPoint?, leftHip: CGPoint?, rightHip: CGPoint?, leftEye: CGPoint?, rightEye: CGPoint?, leftShoulder3D: simd_float3?, rightShoulder3D: simd_float3?, leftElbow3D: simd_float3?, rightElbow3D: simd_float3?
    ) {
        self.init(
            timestamp: timestamp,
            leftWrist: leftWrist,
            ...
            leftEye: leftEye,
            rightEye: rightEye,
            leftShoulder3D: leftShoulder3D,
            rightShoulder3D: rightShoulder3D,
            leftElbow3D: leftElbow3D,
            rightElbow3D: rightElbow3D,
            leftShoulderConfidence: 0.0,
            rightShoulderConfidence: 0.0,
            leftElbowConfidence: 0.0,
            rightElbowConfidence: 0.0,
            leftWristConfidence: 0.0,
            rightWristConfidence: 0.0,
            noseConfidence: 0.0,
            neckConfidence: 0.0
        )
    }

    /// Additional convenience initializer which allows specifying wrist confidences inline (legacy call-sites)
    init(
        timestamp: TimeInterval,
        leftWrist: CGPoint?, rightWrist: CGPoint?, leftElbow: CGPoint?, rightElbow: CGPoint?, leftShoulder: CGPoint?, rightShoulder: CGPoint?, nose: CGPoint?, neck: CGPoint?, leftHip: CGPoint?, rightHip: CGPoint?, leftEye: CGPoint?, rightEye: CGPoint?, leftShoulder3D: simd_float3?, rightShoulder3D: simd_float3?, leftElbow3D: simd_float3?, rightElbow3D: simd_float3?, leftWristConfidence: Float, rightWristConfidence: Float, noseConfidence: Float
    ) {
        self.init(
            timestamp: timestamp,
            leftWrist: leftWrist,
    ...
    func calculateShoulderAbduction(side: BodySide) -> Double? {
        let (shoulder, elbow, hip) =
            side == .left
            ? (leftShoulder, leftElbow, leftHip) : (rightShoulder, rightElbow, rightHip)
        ...
        if let oppositeShoulder = oppositeShoulder {
            ...
            let shoulderLine = CGPoint(
                x: oppositeShoulder.x - shoulder.x, y: oppositeShoulder.y - shoulder.y)
    ...
    func getBestArmpitROM() -> Double? {
        ...
        let rightConfidence =
            (rightShoulderConfidence * rightElbowConfidence) * rightPresencePenalty
    ...
    private func getLandmarksForSide(_ side: BodySide) -> (
        shoulder: CGPoint?, elbow: CGPoint?, confidence: Float
    ) {
        switch side {
        case .left:
        ...
        case .both:
            // For .both, prefer the side with higher confidence
            if rightElbowConfidence >= leftElbowConfidence {
                return (rightShoulder, rightElbow, rightElbowConfidence)
            } else {
                return (leftShoulder, leftElbow, leftElbowConfidence)
            }
    ...
    private func calculateTorsoVertical(leftShoulder: CGPoint, rightShoulder: CGPoint) -> CGPoint {
        ...
        let shoulderLength = sqrt(
            shoulderVector.x * shoulderVector.x + shoulderVector.y * shoulderVector.y)
    ...
    private func calculateAngle(armVector: CGPoint, referenceVector: CGPoint) -> Double {
        ...
        let refMag = sqrt(
            referenceVector.x * referenceVector.x + referenceVector.y * referenceVector.y)
    ...
    func getPoseQuality() -> Double {
        let criticalLandmarks = [
            leftShoulderConfidence, rightShoulderConfidence, leftElbowConfidence,
    ...
    func isValidForROMCalculation() -> Bool {
        ...
        let hasShoulders =
            leftShoulder != nil && rightShoulder != nil && leftShoulderConfidence >= 0.6
            && rightShoulderConfidence >= 0.6
        let hasAtLeastOneElbow =
            (leftElbow != nil && leftElbowConfidence >= 0.6)
